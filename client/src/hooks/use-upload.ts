...
export function useUpload(options: UseUploadOptions = {}) {
  ...
  /**
   * Upload via backend: try local POST /api/uploads first, then presigned URL if needed.
   */
    ...
    async (file: File): Promise<UploadResponse> => {
      ...
      if (localRes.ok) {
        ...
        return {
          ...
          metadata: data.metadata ?? { name: file.name, size: file.size, contentType: file.type || "application/octet-stream" },
        };
      }
